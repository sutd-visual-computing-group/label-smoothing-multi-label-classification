...
def encode_labels_smooth(target, factor = 0.1): #Multi-Hot encoding + label smoothing
    """
    Encode multiple labels using 1/0 encoding 
    Args:
        target: xml tree file
    ...
    ls = target['annotation']['object'] #from xml tree file obtain the spec annotation and object
  
    j = [] #stores indexes or keywords
    if type(ls) == dict:
        if int(ls['difficult']) == 0:
            j.append(object_categories.index(ls['name'])) #append to j the index keyword of a particular class
    else:
        for i in range(len(ls)):
            if int(ls[i]['difficult']) == 0:
                j.append(object_categories.index(ls[i]['name']))
    
    k = np.zeros(len(object_categories)) #the idea is to iterate over all entries in j instead of just a single one
    for element in j:
    	k[element] = 1
...
#LABEL SMOOTHING PART (HERE)
#labels contains one-hot encoded labels for all data points in the dataset
#Version 1
...
"""
#Smoothing Labels version 2
def smooth_labels(labels, factor = 0.1):
	#smoothing the labels, keeping incorrect labels at 0 without adding confidence to the incorrect classes
	labels *= (1 - factor)
	#return smoothed labels
	return labels
"""
